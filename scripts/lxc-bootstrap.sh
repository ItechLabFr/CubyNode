...
if [[ "${EUID}" -ne 0 ]]; then
  echo "This bootstrap must run as root inside the LXC." >&2
  exit 1
fi

export DEBIAN_FRONTEND=noninteractive
apt-get update
apt-get install -y ca-certificates curl git sudo postgresql postgresql-contrib openssl python3 util-linux xz-utils
...
if ! sudo -u postgres psql -tAc "select 1 from pg_database where datname='cubynode'" | grep -q 1; then
